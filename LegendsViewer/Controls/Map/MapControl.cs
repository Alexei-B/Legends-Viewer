...
namespace LegendsViewer.Controls
{
    ...
    public class MapPanel : Panel
    {
        ...
        public bool ZoomToBoundsOnFirstPaint = false, CivsToggled = false, SitesToggled = false, WarsToggled = false, BattlesToggled = false, BattleMapToggled = false, OverlayToggled = false, AlternateMapToggled = false;
        ...
        {
            g.FillRectangle(new SolidBrush(Color.FromArgb(0, 0, 50)), new Rectangle(this.Location, this.Size));
        ...
        private void DrawMiniMap(Graphics g)
        {
            ...
            Rectangle minimapArea = new Rectangle(0, this.Height - MiniMapAreaSideLength, MiniMapAreaSideLength, MiniMapAreaSideLength);
        ...
        public void DrawDisplayObjects(Graphics g)
        {
            ...
            if (FocusObject != null && FocusObject.GetType() == typeof(SiteConquered)) DrawBattlePaths(g, (FocusObject as SiteConquered).Battle);
        ...
        private void DrawInfo(Graphics g)
        {
            ...
            Rectangle tileInfoBox = new Rectangle(ControlMenu.MenuBox.Width, this.Height - Minimap.Height - Convert.ToInt32(tileInfoSize.Height), 70, Convert.ToInt32(tileInfoSize.Height));
            ...
            Rectangle yearBox = new Rectangle(Minimap.Width, this.Height - YearMenu.MenuBox.Height - Convert.ToInt32(yearBoxSize.Height), YearMenu.MenuBox.Width, Convert.ToInt32(yearBoxSize.Height));
        ...
        public void ToggleBattles()
        {
            ...
            else
            {
                ...
                if (FocusObject != null && FocusObject.GetType() == typeof(War))
                {
                    ...
                    Battles.AddRange((FocusObject as War).Collections.OfType<Battle>());
        ...
        public void MakeOverlay(string overlay)
        {
            ...
            switch (overlay)
            {
                ...
                case "Site Events (Filtered)":
                    ...
                    {
                        coordinatesList.Add(coordinates);
                        occurences.Add(World.Sites.Where(site => site.Coordinates == coordinates).Sum(site => site.Events.Count(dEvent => !LegendsViewer.Legends.Site.Filters.Contains(dEvent.Type))));
                    }
                    break;
                case "Beast Attacks":
            ...
            {
                //Point coordinates = coordinatesList[i];
                //coordinatesList[i].X = coordinatesList[i].X * TileSize + TileSize / 2;
                //coordinatesList[i].Y = coordinatesList[i].Y * TileSize + TileSize / 2;
        ...
        public void ChangeMap()
        {
            ...
            {
                deleteFile = true;
                using (SevenZip.SevenZipExtractor extractor = new SevenZip.SevenZipExtractor(openMap.FileName))
                {
                    ...
                    else if (extractor.ArchiveFileNames.Count(file => file.EndsWith(".bmp") || file.EndsWith(".png") || file.EndsWith(".jpg") || file.EndsWith(".jpeg")) == 1)
        ...
        {
            Source.Location = new Point(Center.X - Convert.ToInt32((this.Width / 2) * ZoomCurrent), Center.Y - Convert.ToInt32((this.Height / 2) * ZoomCurrent));
            Source.Size = new Size(Convert.ToInt32(this.Width * ZoomCurrent), Convert.ToInt32(this.Height * ZoomCurrent));
            PixelWidth = Convert.ToDouble(this.Width) / Source.Width;
            PixelHeight = Convert.ToDouble(this.Height) / Source.Height;
            HoverMenu.Open = false;
            ControlMenu.MenuBox.Location = new Point(0, this.Height - Minimap.Height - ControlMenu.MenuBox.Height);
            YearMenu.MenuBox.Location = new Point(Minimap.Width, this.Height - YearMenu.MenuBox.Height);
            AltMapTransparency.Location = new Point(MiniMapAreaSideLength + YearMenu.MenuBox.Width, this.Height - AltMapTransparency.Height);
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (!ControlMenu.HighlightOption(e.X, e.Y) && !YearMenu.HighlightOption(e.X, e.Y) && !OptionsMenu.HighlightOption(e.X, e.Y) && !HoverMenu.Open)
            {
                ...
                if (FocusObject != null && FocusObject.GetType() == typeof(Battle) && (FocusObject as Battle).Coordinates == tile) addOptions.Add(FocusObject as Battle);
